...
void
rotozoomSurfaceSizeTrig(int width, int height, double angle, double zoom,
                        int *dstwidth, int *dstheight, double *canglezoom,
                        double *sanglezoom)
{
    ...
    *canglezoom *= zoom;
    ...
    sy = *sanglezoom * y;
    dstwidthhalf = MAX(
        (int)ceil(MAX(MAX(MAX(fabs(cx + sy), fabs(cx - sy)), fabs(-cx + sy)),
                      fabs(-cx - sy))),
        1);
    dstheighthalf = MAX(
        (int)ceil(MAX(MAX(MAX(fabs(sx + cy), fabs(sx - cy)), fabs(-sx + cy)),
                      fabs(-sx - cy))),
        1);
...
void
zoomSurfaceSize(int width, int height, double zoomx, double zoomy,
                int *dstwidth, int *dstheight)
{
    /*
...
rotozoomSurface(SDL_Surface *src, double angle, double zoom, int smooth)
{
    ...
    if (fabs(angle) > VALUE_LIMIT) {
        ...
        rotozoomSurfaceSizeTrig(rz_src->w, rz_src->h, angle, zoom, &dstwidth,
                                &dstheight, &canglezoom, &sanglezoom);
         ...
         * Calculate target size
         */
        zoomSurfaceSize(rz_src->w, rz_src->h, zoom, zoom, &dstwidth,
                        &dstheight);
         ...
         */
        /*
         * Call the 32bit transformation routine to do the zooming (using
         * alpha)
         */
